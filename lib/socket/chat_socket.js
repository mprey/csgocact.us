      ...
      async.series([
        ...
        function(callback) {
          ...
          if (data.length > 0) {
            ...
            callback(new Error('no data locally'));
          }
        }
        ...
        if (!err) {
          ...
          socket.emit(socket_outgoing.ALERT, {
            header: 'Chat Error',
            type: 'error',
            message: 'Unable to load previous messages. Error: ' + err.message
          });
        }
      });
    } else {
        ...
        if (!err && user) {
          ...
          chatModel.save(function(err) {
            if (err) {
              socket.emit(socket_outgoing.ALERT, {
                type: 'error',
                message: 'Error while saving your chat to the database. Please contact the developers.',
                header: 'Chat Error'
              });
            }
          });
