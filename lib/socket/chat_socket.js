    ...
    if (recentMessages.length < RECENT_MESSAGE_CAP) { //we need to query the database for messages if we dont have enough recent messages (very rarely)
      ...
      async.series([
        ...
        if (!err) {
          ...
          _.sendError(socket, 'Chat Error', 'Unable to load previous messages. Error: ' + err.message);
        }
      });
    } else {
      ...
      User.findById(socket.request.session.passport.user, function(err, user) {
        if (!err && user) {
          ...
          chatModel.save(function(err) {
            if (err) {
              _.sendError(socket, 'Chat Error', 'Error while saving your chat to the database. Please contact the developers.');
            }
          });
        } else {
          _.sendError(socket, 'Chat Error', 'Error while processing. Please contact developers with this message: ' + err.message);
        }
      });
    }
  });
