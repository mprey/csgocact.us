...
module.exports = function(socket, io, _) {
  socket.on(socket_incoming.MUTE_USER, function(data) { //data.muted_id, data.expire, data.reason
    if (_.ensureAuthenticated()) {
      User.findById(socket.request.session.passport.user, function(err, muter) {
        if (!err && muter) {
          if (muter.rank == ranks.ADMIN || muter.rank == ranks.DEVELOPER || muter.rank == ranks.MOD) {
            ...
            _.sendError('Permission Error', 'You do not have permission to mute users.');
          }
        } else {
          _.sendError('User Error', 'Unable to find profile data. Please logout and login.');
        }
      });
    }
  });
      ...
      User.findById(socket.request.session.passport.user, function(err, banner) {
        if (!err && banner) {
          if (banner.rank == ranks.ADMIN || banner.rank == ranks.DEVELOPER) {
            ...
            _.sendError('Permission Error', 'You do not have permission to ban users.');
          }
        } else {
          _.sendError('User Error', 'Unable to find profile data. Please logout and login.');
        }
