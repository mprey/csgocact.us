var coinflipManager = require('./../../../manager/games/coinflip');
  ...
  socket.on(socket_incoming.COINFLIP_CREATE_GAME, function(data, callback) {
    if (_.ensureAuthenticated(true)) {
      _.getUser(function(user) {
        ...
        if (!(data.amount >= config.minimumBet && data.amount <= config.maximumBet)) {
          return callback('Bet is not within ' + config.minimumBet + ' and ' + config.maximumBet + ' bounds');
        }
